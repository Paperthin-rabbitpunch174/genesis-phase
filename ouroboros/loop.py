...
def run_llm_loop(
    ...
    # Upgrade 4: Dynamic round cap — reduce max_rounds for task types
    # that historically finish in fewer rounds. Uses max-observed + 50% buffer
    # instead of a magic multiplier on the average.
    # Floor: 10 rounds (never lower). Ceiling: env MAX_ROUNDS.
    # Requires >= 3 historical tasks to activate (avoids premature capping).
    if drive_root and task_type:
        try:
            from ouroboros.task_stats import get_rounds_stats_for_type
            hist = get_rounds_stats_for_type(drive_root, task_type)
            if hist is not None:
                # Cap = max observed rounds + 50% buffer, floored at 10
                dynamic_cap = max(10, min(MAX_ROUNDS, int(hist["max"] * 1.5)))
                if dynamic_cap < MAX_ROUNDS:
                    log.info(
                        "Dynamic round cap: %s (avg=%d, max=%d, n=%d) → cap=%d (env max=%d)",
                        task_type, hist["avg"], hist["max"], hist["count"],
                        dynamic_cap, MAX_ROUNDS,
                    )
                    MAX_ROUNDS = dynamic_cap
        except Exception:
            pass  # Fall back to env/default MAX_ROUNDS
    round_idx = 0
    try:
        while True:
            round_idx += 1
            ...
            # Upgrade 6: Live cost injection every 10 rounds — LLM paces itself
            if round_idx > 1 and round_idx % 10 == 0:
                task_cost = float(accumulated_usage.get("cost") or 0)
                budget_info = ""
                if budget_remaining_usd is not None:
                    remaining_now = max(0, budget_remaining_usd - task_cost)
                    budget_info = f" Budget remaining: ${remaining_now:.2f}."
                messages.append({
                    "role": "system",
                    "content": (
                        f"[COST CHECK] Round {round_idx}/{MAX_ROUNDS}. "
                        f"This task has spent ${task_cost:.4f}.{budget_info} "
                        f"If nearly done, wrap up. If stuck, try a different approach."
                    ),
                })
