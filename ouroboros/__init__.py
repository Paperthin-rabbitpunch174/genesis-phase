"""
GENESIS PHASE — self-modifying AI agent.
Based on Ouroboros by razzant.

NOTE: This Python package is named 'ouroboros' for import compatibility
with the original codebase. All user-facing text, prompts, and
documentation use the name 'GENESIS PHASE'. Do not rename this
package — it would break all internal imports for zero user benefit.

Philosophy: BIBLE.md
Architecture: agent.py (orchestrator), tools/ (plugin tools),
              llm.py (LLM client), memory.py (memory), review.py (deep review),
              utils.py (shared utilities).
Upgrades:     task_stats.py (outcome intelligence), failure_memory.py (past failure injection).
"""
