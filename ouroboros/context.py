...
def build_llm_messages(
    ...
    base_prompt = _safe_read(
        ...
        fallback="You are GENESIS PHASE. Your base prompt could not be loaded."
    )
    ...
    # Upgrade 5: Past failure injection — LLM reads this and avoids repeating mistakes
    try:
        from ouroboros.failure_memory import get_recent_failures
        failure_warning = get_recent_failures(
            drive_root=env.drive_root,
            task_type=task_type,
        )
        if failure_warning:
            dynamic_parts.append("## Recent Failures\n\n" + failure_warning)
    except Exception:
        log.debug("Failed to load failure memory", exc_info=True)

    # Upgrade 7: Owner preferences — LLM treats these as instructions
    try:
        prefs_path = env.drive_path("memory/owner_preferences.md")
        if prefs_path.exists():
            prefs_text = prefs_path.read_text(encoding="utf-8").strip()
            if prefs_text and len(prefs_text) > 20:
                dynamic_parts.append(
                    "## Owner Preferences\n\n"
                    "These are observed preferences of the creator. Follow them.\n\n"
                    + clip_text(prefs_text, 3000)
                )
    except Exception:
        log.debug("Failed to load owner preferences", exc_info=True)
