...
def _run_pre_push_tests(ctx: ToolContext) -> Optional[str]:
    ...
    Default: OFF ("0"). Pre-existing code quality tests (line length, complexity)
    block ALL pushes on fresh installs. Enable with OUROBOROS_PRE_PUSH_TESTS=1
    after the codebase passes tests.
    """
    if ctx is None:
    ...
    timeout_sec = int(os.environ.get("OUROBOROS_TEST_TIMEOUT_SEC", "60"))
    ...
    try:
        ...
        # Log test result for task stats
        ...
        return f"⚠️ PRE_PUSH_TEST_ERROR: pytest timed out after {timeout_sec}s"
