"""
GENESIS PHASE — Tool registry (SSOT).
...
class ToolRegistry:
    ...
    def __init__(self, repo_dir: pathlib.Path, drive_root: pathlib.Path):
        ...
        self._stats_annotated = False
        self._cached_core_schemas = None
        self._load_modules()
    ...
    def set_context(self, ctx: ToolContext) -> None:
        self._ctx = ctx
        # Reset annotation cache — new task may have new stats
        self._stats_annotated = False
        self._cached_core_schemas = None

    def register(self, entry: ToolEntry) -> None:
        """Register a new tool (for extension by GENESIS PHASE)."""
        self._entries[entry.name] = entry
    ...
    def schemas(self, core_only: bool = False) -> List[Dict[str, Any]]:
        ...
        for e in self._entries.values():
            if e.name in CORE_TOOL_NAMES or e.name in ("list_available_tools", "enable_tools"):
                result.append({"type": "function", "function": dict(e.schema)})

        # Upgrade 8: Annotate tool descriptions with success rates.
        # Cached per task (set_context resets). Don't recompute every round.
        if not self._stats_annotated:
            self._annotate_tool_stats(result)
            self._stats_annotated = True
            self._cached_core_schemas = result
        else:
            result = self._cached_core_schemas

        return result

    def _annotate_tool_stats(self, schemas: List[Dict[str, Any]]) -> None:
        """Append success rate to tool descriptions so LLM picks reliable tools."""
        try:
            from ouroboros.task_stats import compute_tool_stats
            stats = compute_tool_stats(self._ctx.drive_root)
            if not stats:
                return
            for schema in schemas:
                ...
                if name in stats:
                    s = stats[name]
                    if s["calls"] >= 5:  # only annotate with enough data
                        rate = s["success_rate_pct"]
                        note = f" (Recent: {rate}% success across {s['calls']} calls)"
                        if rate < 70:
                            note = f" (⚠️ Recent: {rate}% success, {s['errors']} errors in {s['calls']} calls)"
                        desc = fn.get("description", "")
                        fn["description"] = desc + note
        except Exception:
            pass  # Never break tool loading for stats
