...
def ensure_repo_present() -> None:
    ...
    subprocess.run(["git", "config", "user.name", "GENESIS PHASE"], cwd=str(REPO_DIR), check=True)
...
def checkout_and_reset(branch: str, reason: str = "unspecified",
    ...
    if rc_verify != 0:
        # Branch doesn't exist on remote — create it from current HEAD
        log.info("Branch %s not found on remote — creating from HEAD", branch)
        rc_create = subprocess.run(
            ["git", "checkout", "-b", branch],
            cwd=str(REPO_DIR), capture_output=True,
        ).returncode
        if rc_create != 0:
            # Branch may exist locally but not on remote — just checkout
            subprocess.run(
                ["git", "checkout", branch],
                cwd=str(REPO_DIR), capture_output=True,
            )
        ...
        if rc_push == 0:
            log.info("Created and pushed branch %s", branch)
            append_jsonl(
                DRIVE_ROOT / "logs" / "supervisor.jsonl",
                {
                    "ts": datetime.datetime.now(datetime.timezone.utc).isoformat(),
                    "type": "branch_auto_created",
                    "branch": branch, "reason": reason,
                },
            )
            return True, ""
        else:
            msg = f"Branch {branch} not found and auto-create failed"
            append_jsonl(
                DRIVE_ROOT / "logs" / "supervisor.jsonl",
                {
                    "ts": datetime.datetime.now(datetime.timezone.utc).isoformat(),
